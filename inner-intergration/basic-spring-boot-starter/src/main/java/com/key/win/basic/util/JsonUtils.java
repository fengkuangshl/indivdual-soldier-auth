...
public class JsonUtils {
     ...
     * @throws IOException
     */
    public static <T> T toObject(String jsonString, Class<T> rspValueType) throws JsonParseException, JsonMappingException, IOException {
     ...
     * <p>
     * 1.普通的方式： 默认是转成timestamps形式的，通过下面方式可以取消timestamps。 objectMapper.configure(SerializationConfig.Feature.WRITE_DATES_AS_TIMESTAMPS, false);
     * 这样将使时间生成使用所谓的use a [ISO-8601 ]-compliant notation, 输出类似如下格式的时间: "1970-01-01T00:00:00.000+0000". 当然也可以自定义输出格式：
     * objectMapper.getSerializationConfig().setDateFormat(myDateFormat); myDateFormat对象为java.text.DateFormat，具体使用清查java API 2.annotaion的注释方式：
     * 先定义自己需要的格式如下类
     * <p>
...
//            return null;
//        }
        JsonNode jsonNode = JsonUtils.readJsonNode(jsonStr);
        if (jsonNode != null) {
    ...
    public static <T> T toObject(String json, Class<T> mainClass, Class<?>... subClass) throws Exception {
        JavaType javaType = objectMapper.getTypeFactory().constructParametricType(mainClass, subClass);
