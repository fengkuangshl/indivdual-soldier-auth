...
export const UserGetApi = (id: string): Promise<KWResponse.Result<UserInfo>> => request.get('user/getUserFullById/' + id)
export const UserSaveOrUpdateApi = (userInfo: UserForm): Promise<KWResponse.Result> => request.post('user/saveOrUpdate', userInfo)
export const ResetPasswordApi = (id: string): Promise<KWResponse.Result<UserInfo>> => request.get('user/resetPassword/' + id)
